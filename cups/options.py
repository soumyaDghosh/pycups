...
def addOption(
    name: str, value: Union[int, str], options: Optional[List[cupsOption]]
) -> Dict[str, cupsOption]:
    c_opts: Any
    if options is None:
        ...
        raise ValueError(
            "options variable needed to be passed if not creating a new options list"
        )
    ...
    count = _lib.cupsAddOption(c_name, c_value, c_num_options, c_opts)
